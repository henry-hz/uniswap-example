...
# This script deploys test tokens, initializes a pool, and adds liquidity
...
if [ -z "$PRIVATE_KEY" ]; then
  ...
  echo "Usage: PRIVATE_KEY=your_private_key ./run_deploy_and_add.sh"
  exit 1
fi
...
# Run the script to deploy tokens and add liquidity
echo "Running DeployTestTokensAndAddLiquidity script..."
forge script script/03_DeployTestTokensAndAddLiquidity.s.sol \
  --rpc-url "$RPC_URL" \
  $PRIVATE_KEY_ARG \
  --broadcast \
  -vvv \
