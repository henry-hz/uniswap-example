...
# This script runs the SepoliaAddLiquidity script using Forge
...
if [ -z "$PRIVATE_KEY" ]; then
  ...
  echo "Usage: PRIVATE_KEY=your_private_key ./run_add_liquidity.sh"
  exit 1
fi
...
# Run the Sepolia add liquidity script
echo "Running SepoliaAddLiquidity script..."
forge script script/02a_SepoliaAddLiquidity.s.sol \
  --rpc-url "$RPC_URL" \
  $PRIVATE_KEY_ARG \
  --broadcast \
  -vvv \
